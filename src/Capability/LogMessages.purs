...
-- | the application monad, `Fpers.AppM`.
-- |
...
module Fpers.Capability.LogMessages where

import Prelude

import Fpers.Capability.Now (class Now)
import Fpers.Data.Log (Log, LogReason(..), mkLog)
...
-- | `Log` type from `Fpers.Data.Log`. This type can only be constructed using helper functions
