...
-- | functions that our production monad will leverage in `Fpers.AppM` to implement our various
-- | app capabilities.
...
import Fpers.Data.Email as Email
...
-- | Fpers uses a REST API and secures certain endpoints using a JSON Web Token (JWT). That means
...
-- |
-- | Check out the `Fpers.Capability.Resource.*` modules for examples of the high-level functions
...
-- | request functions defined below and in the `Fpers.Api.Utils` modules.
...
-- | particular key. They'll be used as part of our production monad, `Fpers.AppM`.
