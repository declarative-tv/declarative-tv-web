-- | Fpers uses a REST API for resource management. This module defines endpoints in a data type
...
-- |
-- | This module takes the same approach as the `Fpers.Data.Route` module. For a more in-depth
...
import Fpers.Data.Comment (CommentId)
import Fpers.Data.Route (slug, uname)
import Fpers.Data.Username (Username)
import Routing.Duplex (RouteDuplex', int, optional, prefix, root, segment, string, many)
...
endpointCodec = root $ prefix "helix" $ sum -- TODO <---
  { -- https://api.twitch.tv/helix/streams?user_login=gillchristian&user_login=cvladfp&user_login=gernaderjake
