...
import Fpers.Api.Request (BaseURL, RequestOptions, Token, defaultRequest, readToken, writeToken)
...
-- | `baseUrl` field from the app environment. See `Fpers.AppM` for examples of this in action.
mkRequest
  :: forall m r
   . MonadAff m
...
-- | the asynchronous request. See `Fpers.AppM` for examples of this in action.
mkAuthRequest
  :: forall m r
   . MonadAff m
  => MonadAsk { baseUrl :: BaseURL | r } m
...
-- | dealing with malformed responses. See `Fpers.AppM` for examples of this in practice.
