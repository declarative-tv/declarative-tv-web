...
module Fpers.Form.Validation where

import Prelude

import Fpers.Data.Avatar (Avatar)
...
-- | `e`, which represents the possible error type which can result from the validator. For Fpers
...
-- | This validator ensures that an input string is a valid username. Usernames in Fpers use the
-- | smart constructor pattern, so we can't construct a username directly -- we'll need to defer
-- | to the `parse` helper function exported by `Fpers.Data.Username`. Since that function returns
